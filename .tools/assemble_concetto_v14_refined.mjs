...
const outDir = path.join(cwd, '06_预览输出', 'refined_v14_parts');
...
const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v14_低清.mp4');
const preview = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v14_全片预览.jpg');
const opPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v14_演示包装预览.jpg');
const aiPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v14_AI渲染成果预览.jpg');
const logPath = path.join(cwd, '03_脚本与结构', 'Concetto 2.0_综合调整_v14_说明.md');
...
function renderIntroRestored() {
  const out = path.join(outDir, 'intro_restored_logo_sweep_to_predesign_v14.mp4');
  run([
    '-y',
    '-ss', '0',
    '-t', '36.3',
    '-i', v10Intro,
    '-vf', `fps=${fps},scale=${W}:${H},setsar=1,fade=t=in:st=0:d=0.18,fade=t=out:st=36.0:d=0.3`,
    '-an',
    '-c:v', 'libx264',
    '-preset', 'medium',
    '-crf', '20',
...
async function main() {
  ...
  const introRestored = renderIntroRestored();
  const parts = [introRestored];
  const records = [
    { segment: '开头：恢复暗场 CONCETTO 扫光、升级亮点、六大更新、九环总览与 01 前策分析开场', file: introRestored, duration: probeDur(introRestored) },
  ];
  ...
  for (const section of sections) {
    if (section.no !== '01') {
      ...
      records.push({ segment: '01 前策分析 章节开头', file: '已包含在恢复的旧 intro 尾段，避免重复生成', duration: 0 });
    }
  ...
  const videoOnly = path.join(outDir, 'video_concat_v14.mp4');
  run(['-y', '-f', 'concat', '-safe', '0', '-i', list, '-c', 'copy', videoOnly], 'concat v14');
  ...
  run(['-y', '-i', output, '-vf', 'fps=0.24,scale=360:-1,tile=10x7', '-frames:v', '1', preview], 'preview v14');
  run(['-y', '-ss', '00:00:38', '-i', output, '-t', '00:00:32', '-vf', 'fps=1,scale=480:-1,tile=8x4', '-frames:v', '1', opPreview], 'operation preview v14');
  run(['-y', '-ss', '00:01:28', '-i', output, '-t', '00:00:42', '-vf', 'fps=1,scale=480:-1,tile=10x5', '-frames:v', '1', aiPreview], 'ai preview v14');

  fs.writeFileSync(logPath, [
    '# Concetto 2.0 综合调整 v14',
    '',
    '本版按最新反馈处理：',
    '',
    '- 恢复最初暗场 CONCETTO 登场与左到右扫光段落；',
    '- 开头沿用旧 intro 的升级亮点、六大更新、九环总览，只保留一份九环总览，删除 v13 中额外生成的重复总览；',
    '- 恢复 01 前策分析章节开场，并直接接入新版放大后的操作演示；',

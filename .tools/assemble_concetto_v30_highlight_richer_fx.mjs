...
const outDir = path.join(cwd, '06_预览输出', 'refined_v30_highlight_richer_fx_parts');
const frameDir = path.join(outDir, 'frames_update_content_v30');
const highlightFrameDir = path.join(outDir, 'frames_highlight_v30');
const updateDir = path.join(cwd, '更新内容');
const highlightDir = path.join(cwd, '亮点');
const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v30_升级亮点呼吸光效增强_低清.mp4');
const preview = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v30_升级亮点呼吸光效增强_全片预览.jpg');
const introPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v30_前置段预览.jpg');
const updatePreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v30_六大模块段预览.jpg');
const highlightPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v30_升级亮点段预览.jpg');
const logPath = path.join(cwd, '03_脚本与结构', 'Concetto 2.0_v30_升级亮点呼吸光效增强说明.md');
...
async function main() {
  ...
  const updateClip = path.join(cwd, '06_预览输出', 'refined_v29_highlight_update_fx_parts', 'intro_update_contents_v29_card_edge_fx.mp4');
  assertFile(updateClip);
  ...
  const list = path.join(outDir, 'concat_list_v30.txt');
  fs.writeFileSync(list, parts.map((p) => `file '${quoteFile(p)}'`).join('\n') + '\n');

  const videoOnly = path.join(outDir, 'video_concat_v30_reencoded.mp4');
  run(ffmpeg, [
    '-y',
    '-f', 'concat',
    '-safe', '0',
  ...
  run(ffmpeg, ['-y', '-i', output, '-vf', 'fps=0.24,scale=360:-1,tile=10x7', '-frames:v', '1', preview], 'preview v30');
  run(ffmpeg, ['-y', '-ss', '00:00:00', '-i', output, '-t', '00:00:45', '-vf', 'fps=1,scale=480:-1,tile=10x5', '-frames:v', '1', introPreview], 'intro preview v30');
  run(ffmpeg, ['-y', '-i', updateClip, '-vf', 'fps=1,scale=480:-1,tile=8x1', '-frames:v', '1', updatePreview], 'update preview v30');
  run(ffmpeg, ['-y', '-i', highlightClip, '-vf', 'fps=1,scale=480:-1,tile=8x1', '-frames:v', '1', highlightPreview], 'highlight preview v30');

  fs.writeFileSync(logPath, [
    '# Concetto 2.0 v30 升级亮点呼吸光效增强',
    '',
    '- 基于 v27 成片继续制作，替换“升级亮点”片段，更新内容段沿用 v29 柔和扫光版本；',
    '- 升级亮点素材来自项目文件夹 `亮点`，保留原图片内字体和页面布局；',
    '- 四个亮点没有先后关系，改为并行呼吸描边、游走高光、角点脉冲、图标光晕、卡片内部玻璃呼吸层、微星点与细光丝漂移；',
    '- 升级亮点扫光进一步压低入场强度，主要亮度集中在中段，离开时自然衰减；',
    '- 更新内容段沿用 v29 描边与柔和扫光逻辑；',

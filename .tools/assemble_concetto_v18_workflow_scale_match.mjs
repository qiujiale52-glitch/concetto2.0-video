...
const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环尺度匹配_v18_低清.mp4');
const preview = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环尺度匹配_v18_全片预览.jpg');
const opPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环尺度匹配_v18_演示包装预览.jpg');
const aiPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环尺度匹配_v18_AI渲染成果预览.jpg');
const introPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环尺度匹配_v18_前置段检查.jpg');
const logPath = path.join(cwd, '03_脚本与结构', 'Concetto 2.0_九环尺度匹配_v18_说明.md');
...
async function renderWorkflowOverview() {
  const dur = 7.2;
  const dir = path.join(frameRoot, 'workflow_overview_v18');
  fs.mkdirSync(dir, { recursive: true });
  const count = Math.round(dur * fps);
  for (let i = 0; i < count; i++) {
    const out = path.join(dir, `frame_${String(i).padStart(4, '0')}.png`);
  ...
  const out = path.join(outDir, 'intro_workflow_overview_v18.mp4');
  run(['-y', '-framerate', String(fps), '-i', path.join(dir, 'frame_%04d.png'), '-c:v', 'libx264', '-preset', 'medium', '-crf', '20', '-pix_fmt', 'yuv420p', '-r', String(fps), out], 'workflow overview v18');
  return out;
}
...
async function main() {
  ...
  const records = [
    ...
    { segment: '前置段 03：九大环节最新版图片重新组织动效 v18（尺度匹配亮点段）', file: workflowOverview, duration: probeDur(workflowOverview) },
  ];
  ...
  const videoOnly = path.join(outDir, 'video_concat_v18.mp4');
  run(['-y', '-f', 'concat', '-safe', '0', '-i', list, '-c', 'copy', videoOnly], 'concat v18');
  ...
  run(['-y', '-i', output, '-vf', 'fps=0.24,scale=360:-1,tile=10x7', '-frames:v', '1', preview], 'preview v18');
  run(['-y', '-ss', '00:00:00', '-i', output, '-t', '00:00:32', '-vf', 'fps=1,scale=480:-1,tile=8x4', '-frames:v', '1', introPreview], 'intro preview v18');
  run(['-y', '-ss', '00:00:31', '-i', output, '-t', '00:00:32', '-vf', 'fps=1,scale=480:-1,tile=8x4', '-frames:v', '1', opPreview], 'operation preview v18');
  run(['-y', '-ss', '00:01:24', '-i', output, '-t', '00:00:42', '-vf', 'fps=1,scale=480:-1,tile=10x5', '-frames:v', '1', aiPreview], 'ai preview v18');

  fs.writeFileSync(logPath, [
    '# Concetto 2.0 九大环节尺度匹配 v18',
    '',
    '本版按最新反馈处理：',
    '',

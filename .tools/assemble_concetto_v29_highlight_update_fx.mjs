...
const outDir = path.join(cwd, '06_预览输出', 'refined_v29_highlight_update_fx_parts');
const frameDir = path.join(outDir, 'frames_update_content_v29');
const highlightFrameDir = path.join(outDir, 'frames_highlight_v29');
const updateDir = path.join(cwd, '更新内容');
const highlightDir = path.join(cwd, '亮点');
const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v29_亮点与六大模块光效精修_低清.mp4');
const preview = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v29_亮点与六大模块光效精修_全片预览.jpg');
const introPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v29_前置段预览.jpg');
const updatePreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v29_六大模块段预览.jpg');
const highlightPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v29_升级亮点段预览.jpg');
const logPath = path.join(cwd, '03_脚本与结构', 'Concetto 2.0_v29_亮点与六大模块光效精修说明.md');
...
function renderHighlightSvg(t, dur, imgFile, meta) {
  ...
  const scanStrength = Math.pow(Math.sin(Math.PI * globalPos), 1.28);
  ...
  const cardEffects = cards.map((r, i) => {
    ...
    const sweepA = Math.pow(Math.sin(Math.PI * sweepPos), 1.22) * smooth(0, 0.10, local) * (1 - smooth(0.90, 1.0, local));
    ...
    return `<g opacity="${alpha.toFixed(3)}">
      ...
      <path d="${d}" pathLength="1000" fill="none" stroke="url(#edgeGrad)" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round" stroke-opacity="${(0.34 + 0.26 * drawA).toFixed(3)}" filter="url(#pinGlow)"/>
      ...
      <rect x="${sweepX.toFixed(1)}" y="${(r.y - 38).toFixed(1)}" width="${(r.w * (0.19 + 0.06 * breath)).toFixed(1)}" height="${(r.h + 76).toFixed(1)}" transform="skewX(-17)" fill="url(#lineGrad)" opacity="${(0.31 * sweepA * (0.74 + 0.26 * breath)).toFixed(3)}"/>
  ...
  <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="${globalScan.toFixed(1)}" y="-90" width="700" height="1260" transform="skewX(-18)" fill="url(#lineGrad)" opacity="${(0.075 * a * scanStrength * (0.75 + 0.25 * Math.sin(t * 1.8))).toFixed(3)}"/>
...
async function renderHighlightClip() {
  ...
  const img = imgs[0];
  const meta = await sharp(img).metadata();
  const lightImg = path.join(outDir, 'highlight_source_scaled_v29.jpg');
  await sharp(img)
  ...
  const out = path.join(outDir, 'intro_highlight_01_v29_feature_edge_fx.mp4');
  run(ffmpeg, [
    '-y',
    '-framerate', String(FPS),
...
async function main() {
  ...
  const list = path.join(outDir, 'concat_list_v29.txt');
  fs.writeFileSync(list, parts.map((p) => `file '${quoteFile(p)}'`).join('\n') + '\n');

  const videoOnly = path.join(outDir, 'video_concat_v29_reencoded.mp4');
  run(ffmpeg, [
    '-y',
    '-f', 'concat',
    '-safe', '0',
  ...
  run(ffmpeg, ['-y', '-i', output, '-vf', 'fps=0.24,scale=360:-1,tile=10x7', '-frames:v', '1', preview], 'preview v29');
  run(ffmpeg, ['-y', '-ss', '00:00:00', '-i', output, '-t', '00:00:45', '-vf', 'fps=1,scale=480:-1,tile=10x5', '-frames:v', '1', introPreview], 'intro preview v29');
  run(ffmpeg, ['-y', '-i', updateClip, '-vf', 'fps=1,scale=480:-1,tile=8x1', '-frames:v', '1', updatePreview], 'update preview v29');
  run(ffmpeg, ['-y', '-i', highlightClip, '-vf', 'fps=1,scale=480:-1,tile=8x1', '-frames:v', '1', highlightPreview], 'highlight preview v29');

  fs.writeFileSync(logPath, [
    '# Concetto 2.0 v29 亮点与六大模块光效精修',
    '',
    '- 基于 v27 成片继续制作，替换“升级亮点”和“更新内容 / 六大模块”片段；',
    '- 升级亮点素材来自项目文件夹 `亮点`，保留原图片内字体和页面布局；',
    '- 四个亮点没有先后关系，改为并行呼吸描边、游走高光、角点脉冲、图标光晕与粒子漂移；',
    '- 更新内容段沿用 v28 描边逻辑，但扫光强度改为淡入→中段增强→离开减弱；',

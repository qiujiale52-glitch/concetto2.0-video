...
const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_前策前重做_v16_低清.mp4');
const preview = path.join(cwd, '06_预览输出', 'Concetto_2.0_前策前重做_v16_全片预览.jpg');
const opPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_前策前重做_v16_演示包装预览.jpg');
const aiPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_前策前重做_v16_AI渲染成果预览.jpg');
const introPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_前策前重做_v16_前置段检查.jpg');
const logPath = path.join(cwd, '03_脚本与结构', 'Concetto 2.0_前策前重做_v16_说明.md');
...
async function renderWorkflowOverview() {
  const dur = 7.2;
  const dir = path.join(frameRoot, 'workflow_overview_v16');
  fs.mkdirSync(dir, { recursive: true });
  const count = Math.round(dur * fps);
  for (let i = 0; i < count; i++) {
    const out = path.join(dir, `frame_${String(i).padStart(4, '0')}.png`);
  ...
  const out = path.join(outDir, 'intro_workflow_overview_v16.mp4');
  run(['-y', '-framerate', String(fps), '-i', path.join(dir, 'frame_%04d.png'), '-c:v', 'libx264', '-preset', 'medium', '-crf', '20', '-pix_fmt', 'yuv420p', '-r', String(fps), out], 'workflow overview v16');
  return out;
}
...
async function main() {
  ...
  const openingLogo = renderOpeningFromFolder();
  const introHighlights = await renderHighlightsIntro();
  const workflowOverview = await renderWorkflowOverview();
  const parts = [openingLogo, ...introHighlights, workflowOverview];
  const records = [
    { segment: '前置段 01：开头文件夹视频直接使用', file: openingLogo, duration: probeDur(openingLogo) },
    ...introHighlights.map((p, i) => ({ segment: `前置段 02：亮点图片动效 ${i + 1}`, file: p, duration: probeDur(p) })),
    { segment: '前置段 03：九大环节最新版图片 + 路径流光点亮动效', file: workflowOverview, duration: probeDur(workflowOverview) },
  ];
  ...
  const videoOnly = path.join(outDir, 'video_concat_v16.mp4');
  run(['-y', '-f', 'concat', '-safe', '0', '-i', list, '-c', 'copy', videoOnly], 'concat v16');
  ...
  run(['-y', '-i', output, '-vf', 'fps=0.24,scale=360:-1,tile=10x7', '-frames:v', '1', preview], 'preview v16');
  run(['-y', '-ss', '00:00:00', '-i', output, '-t', '00:00:32', '-vf', 'fps=1,scale=480:-1,tile=8x4', '-frames:v', '1', introPreview], 'intro preview v16');
  run(['-y', '-ss', '00:00:31', '-i', output, '-t', '00:00:32', '-vf', 'fps=1,scale=480:-1,tile=8x4', '-frames:v', '1', opPreview], 'operation preview v16');
  run(['-y', '-ss', '00:01:24', '-i', output, '-t', '00:00:42', '-vf', 'fps=1,scale=480:-1,tile=10x5', '-frames:v', '1', aiPreview], 'ai preview v16');

  fs.writeFileSync(logPath, [
    '# Concetto 2.0 前策前重做 v16',
    '',
    '本版按最新反馈处理：',
    '',
    ...
    '- 九大环节总览使用 `九大环节最新版` 图片重新生成，加入沿流程线运动的亮光与图标逐个点亮；',

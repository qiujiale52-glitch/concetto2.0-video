...
const H = 1080;
const FPS = 30;
const srcList = path.join(cwd, '06_预览输出', 'refined_v22_layoutfix_parts', 'concat_list_v22.txt');
const outDir = path.join(cwd, '06_预览输出', 'refined_v23_update_insert_parts');
const frameDir = path.join(outDir, 'frames_update_content');
const updateDir = path.join(cwd, '更新内容');
const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v24_低清.mp4');
const preview = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v24_全片预览.jpg');
const introPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_AI超分操作演示_v24_前置段预览.jpg');
const logPath = path.join(cwd, '03_脚本与结构', 'Concetto 2.0_v24_修复更新内容幽灵字说明.md');
...
function renderUpdateSvg(t, dur, imgFile, meta) {
  ...
  const cardEffects = cards.map((r, i) => {
    const k = smooth(0.58 + i * 0.30, 1.08 + i * 0.30, t);
    const hold = 1 - smooth(dur - 0.85, dur - 0.2, t);
    const pulse = 0.58 + 0.42 * Math.sin(t * 4.4 + i * 1.18);
    const sweep = r.x - r.w * 0.35 + r.w * 1.65 * clamp((t - (1.0 + i * 0.25)) / 1.35);
    const alpha = a * k * hold;
    return `<g opacity="${alpha.toFixed(3)}">
      <rect x="${r.x.toFixed(1)}" y="${r.y.toFixed(1)}" width="${r.w.toFixed(1)}" height="${r.h.toFixed(1)}" rx="24" fill="none" stroke="#ffffff" stroke-width="2.4" stroke-opacity="${(0.22 + 0.32 * pulse).toFixed(3)}" filter="url(#glow)"/>
      <rect x="${sweep.toFixed(1)}" y="${(r.y - 28).toFixed(1)}" width="${(r.w * 0.20).toFixed(1)}" height="${(r.h + 56).toFixed(1)}" transform="skewX(-17)" fill="url(#lineGrad)" opacity="${(0.18 * k).toFixed(3)}"/>
      <circle cx="${(r.x + r.w * 0.14).toFixed(1)}" cy="${(r.y + r.h * 0.30).toFixed(1)}" r="${(22 + 8 * pulse).toFixed(1)}" fill="#ffffff" opacity="${(0.12 + 0.11 * pulse).toFixed(3)}" filter="url(#softGlow)"/>
    </g>`;
  }).join('');
  ...
  <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="${scanX.toFixed(1)}" y="-80" width="620" height="1240" transform="skewX(-18)" fill="url(#lineGrad)" opacity="${(0.10 * a).toFixed(3)}"/>
...
async function renderUpdateClip() {
  ...
  const out = path.join(outDir, 'intro_update_contents_v24.mp4');
  run(ffmpeg, [
    '-y',
...
async function main() {
  ...
  for (const p of baseParts) {
    parts.push(p);
    if (!inserted && path.basename(p) === 'intro_highlight_01_v16.mp4') {
      parts.push(updateClip);
      inserted = true;
    }
  }
  if (!inserted) throw new Error('未找到 intro_highlight_01_v16.mp4，无法插入更新内容段');

  const list = path.join(outDir, 'concat_list_v24.txt');
  fs.writeFileSync(list, parts.map((p) => `file '${quoteFile(p)}'`).join('\n') + '\n');

  const videoOnly = path.join(outDir, 'video_concat_v24_reencoded.mp4');
  run(ffmpeg, [
    '-y',
    '-f', 'concat',
    '-safe', '0',
  ...
  run(ffmpeg, ['-y', '-i', output, '-vf', 'fps=0.24,scale=360:-1,tile=10x7', '-frames:v', '1', preview], 'preview v23');
  run(ffmpeg, ['-y', '-ss', '00:00:00', '-i', output, '-t', '00:00:38', '-vf', 'fps=1,scale=480:-1,tile=10x4', '-frames:v', '1', introPreview], 'intro preview v23');

  fs.writeFileSync(logPath, [
    '# Concetto 2.0 v24 修复更新内容段幽灵字',
    '',
    '- 基于 v22 成片继续制作；',
    '- 在“亮点”与“九大环节总览”之间保留“更新内容”动效；',
    '- 更新内容素材来自项目文件夹 `更新内容`，保留原图片内字体和页面布局，只在外层加入暗场、屏幕卡片、六模块扫光/点亮动效；',
    '- 移除了叠在素材图下方的额外居中标题和副标题，避免 PNG 半透明区域透出“一闪而过”的幽灵字；',
    '- 后续九大环节、操作演示、DWG 成果、文本生成与结尾均沿用 v22。',
    '',
    `新增片段：\`${updateClip}\``,
    `输出视频：\`${output}\``,
    `前置段预览：\`${introPreview}\``,

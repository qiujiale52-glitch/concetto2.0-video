...
const outDir = path.join(cwd, '06_预览输出', 'opening_v03_20_shockwave_frames');
...
function makeSvg(t) {
  ...
  const twoScale = 0.92 + 0.08 * smoothstep(5.0, 6.0, t) + 0.105 * twoHit + 0.028 * twoAfterPulse;
  const twoGlow = Math.min(0.72, 0.22 * twoA + 0.42 * twoHit + 0.20 * twoAfterPulse);
  ...
  function twoWave(start, dur, maxScale, maxOpacity, blur, strokeBase) {
    ...
    const yShift = 620 - 4 * p;
    return `<g opacity="${a.toFixed(3)}" filter="url(#blur${blur})" transform="translate(960 620) scale(${scale.toFixed(4)}) translate(-960 -620)">
      <text x="960" y="${yShift.toFixed(1)}" text-anchor="middle" font-family="Avenir Next, Helvetica Neue, Arial, sans-serif" font-size="72" font-weight="500" fill="none" stroke="#dcd7ff" stroke-width="${sw.toFixed(2)}" stroke-linejoin="round" letter-spacing="8">2.0</text>
    </g>`;
  }

  const twoShockwaves = [
    twoWave(6.10, 1.16, 0.42, 0.62, 6, 1.2),
    twoWave(6.28, 1.10, 0.62, 0.38, 6, 1.0),
    twoWave(6.50, 1.00, 0.85, 0.24, 24, 0.8),
  ].join('\n');
...
<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
  ...
  <g opacity="${shockLineA.toFixed(3)}" filter="url(#blur24)">
    <ellipse cx="960" cy="604" rx="${(140 + 620 * shockLineA).toFixed(1)}" ry="${(3 + 10 * shockLineA).toFixed(1)}" fill="#bcb4ff" opacity="${(0.18 + 0.22 * shockLineA).toFixed(3)}"/>
    <rect x="${(960 - 850 * shockLineA).toFixed(1)}" y="604" width="${(1700 * shockLineA).toFixed(1)}" height="1.8" fill="#d9d4ff" opacity="0.42"/>
  </g>
  ${twoShockwaves}
  <g transform="translate(960 620) scale(${twoScale.toFixed(4)}) translate(-960 -620)">
    <text x="960" y="620" text-anchor="middle" font-family="Avenir Next, Helvetica Neue, Arial, sans-serif" font-size="72" font-weight="500" fill="#d8d2ff" opacity="${twoA.toFixed(3)}" letter-spacing="8" filter="url(#softShadow)">2.0</text>
    <text x="960" y="620" text-anchor="middle" font-family="Avenir Next, Helvetica Neue, Arial, sans-serif" font-size="72" font-weight="500" fill="#ffffff" opacity="${(0.18 * twoGlow).toFixed(3)}" letter-spacing="8">2.0</text>
  </g>

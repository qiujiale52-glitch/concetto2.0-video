...
const outDir = path.join(cwd, '06_预览输出', 'refined_v13_parts');
...
const workflowImage = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环节总览_原版局部重排_v10.jpg');

const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v13_低清.mp4');
const preview = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v13_全片预览.jpg');
const opPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v13_演示包装预览.jpg');
const aiPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v13_AI渲染成果预览.jpg');
const logPath = path.join(cwd, '03_脚本与结构', 'Concetto 2.0_综合调整_v13_说明.md');
...
function operationBgSvg(section) {
  ...
  <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="126" y="108" width="1668" height="940" rx="34" fill="#12102a" opacity="0.72" filter="url(#panelGlow)"/>
    <rect x="126" y="108" width="1668" height="940" rx="34" fill="none" stroke="#8375ff" stroke-opacity="0.42" stroke-width="1.4"/>
    <rect x="150" y="126" width="1620" height="912" rx="24" fill="#f5f5ff" opacity="0.025"/>
...
async function renderOperationClip(section, clip = section.clip, feature = section.feature, suffix = '', opts = {}) {
  ...
  const filter = [
    `[0:v]setpts=PTS-STARTPTS,scale=1620:912:force_original_aspect_ratio=decrease,pad=1620:912:(ow-iw)/2:(oh-ih)/2:color=0x17142f,setsar=1,fps=${fps}[vid]`,
    `[1:v]scale=${W}:${H},setsar=1[bg]`,
    `[bg][vid]overlay=x=150:y=126:format=auto[v0]`,
    `[v0]drawbox=x=150:y=126:w=1620:h=912:color=0x8d7cff@0.18:t=2,fade=t=in:st=0:d=0.18,fade=t=out:st=${Math.max(0.1, dur - 0.25)}:d=0.25[v]`,
  ].join(';');
...
function renderWorkflowOverviewSvg(t, dur = 6.2) {
  const a = smooth(0.15, 0.9, t) * (1 - smooth(dur - 0.55, dur, t));
  const panel = easeOut(t / 1.25);
  const imgUri = dataUri(workflowImage);
  const scale = 0.965 + 0.035 * panel;
  const imgW = 1450 * scale;
  const imgH = 790 * scale;
  const imgX = (W - imgW) / 2;
  const imgY = 230 - 8 * panel;
  ...
  <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="112" y="136" font-family="Avenir Next, Arial" font-size="17" letter-spacing="7" fill="#857be8" opacity="${(0.72 * a).toFixed(3)}">CONCETTO 2.0 / LAUNCH SEQUENCE</text>
    <rect x="112" y="158" width="${(265 * a).toFixed(1)}" height="1.2" fill="#7668ff" opacity="0.55"/>
    <text x="960" y="154" text-anchor="middle" font-family="MiSans, PingFang SC, Arial" font-size="42" font-weight="750" fill="#f2f0ff" opacity="${(0.94 * a).toFixed(3)}">从升级能力，到完整工作流</text>
    <text x="960" y="202" text-anchor="middle" font-family="MiSans, PingFang SC, Arial" font-size="22" letter-spacing="1.2" fill="#aaa2df" opacity="${(0.84 * a).toFixed(3)}">九大环节串联，从任务书解读到汇报材料合成</text>
    <g opacity="${a.toFixed(3)}" filter="url(#glow)">
      <rect x="${(imgX - 26).toFixed(1)}" y="${(imgY - 24).toFixed(1)}" width="${(imgW + 52).toFixed(1)}" height="${(imgH + 48).toFixed(1)}" rx="42" fill="#151039" stroke="#8d7cff" stroke-opacity="0.28"/>
...
async function renderWorkflowOverview() {
  const dur = 6.2;
  const dir = path.join(frameRoot, 'workflow_overview_v13');
  fs.mkdirSync(dir, { recursive: true });
  const count = Math.round(dur * fps);
  for (let i = 0; i < count; i++) {
    const out = path.join(dir, `frame_${String(i).padStart(4, '0')}.png`);
  ...
  const out = path.join(outDir, 'intro_workflow_overview_v13.mp4');
  run(['-y', '-framerate', String(fps), '-i', path.join(dir, 'frame_%04d.png'), '-c:v', 'libx264', '-preset', 'medium', '-crf', '20', '-pix_fmt', 'yuv420p', '-r', String(fps), out], 'workflow overview v13');
  return out;
}

function renderIntroNoLogo() {
  const out = path.join(outDir, 'intro_highlights_updates_no_logo_v13.mp4');
  run([
    '-y',
    '-ss', '12',
    '-t', '17',
    '-i', v10Intro,
    '-vf', `fps=${fps},scale=${W}:${H},setsar=1,fade=t=in:st=0:d=0.18,fade=t=out:st=16.72:d=0.28`,
    '-an',
    '-c:v', 'libx264',
    '-preset', 'medium',
    '-crf', '20',
...
async function main() {
  ...
  const introNoLogo = renderIntroNoLogo();
  const workflowOverview = await renderWorkflowOverview();
  const parts = [introNoLogo, workflowOverview];
  const records = [
    { segment: '开头：删除重复 CONCETTO 2.0 过场，保留升级亮点/更新', file: introNoLogo, duration: probeDur(introNoLogo) },
    { segment: '重新生成九环节总览场景', file: workflowOverview, duration: probeDur(workflowOverview) },
  ];
  ...
  for (const section of sections) {
    if (section.no !== '01') {
      ...
      records.push({ segment: '01 前策分析 章节开头', file: '按最新反馈删除，九环总览后直接进入正式操作演示', duration: 0 });
    }
  ...
  const videoOnly = path.join(outDir, 'video_concat_v13.mp4');
  run(['-y', '-f', 'concat', '-safe', '0', '-i', list, '-c', 'copy', videoOnly], 'concat v13');
  ...
  run(['-y', '-i', output, '-vf', 'fps=0.24,scale=360:-1,tile=10x7', '-frames:v', '1', preview], 'preview v13');
  run(['-y', '-ss', '00:00:23', '-i', output, '-t', '00:00:32', '-vf', 'fps=1,scale=480:-1,tile=8x4', '-frames:v', '1', opPreview], 'operation preview v13');
  run(['-y', '-ss', '00:01:20', '-i', output, '-t', '00:00:42', '-vf', 'fps=1,scale=480:-1,tile=10x5', '-frames:v', '1', aiPreview], 'ai preview v13');

  fs.writeFileSync(logPath, [
    '# Concetto 2.0 综合调整 v13',
    '',
    '本版按最新反馈处理：',
    '',
    '- 删除开头中重复出现的 CONCETTO 2.0 logo 过场，改为升级亮点/更新后直接进入重新生成的九环节总览；',
    '- 九环节总览重新生成一版暗场发布会屏幕，九环顺序保持 01 前策分析、02 场地定位、03 图生模型、04 数智建模、05 AI灵感渲染、06 总图排布、07 AI仿真分析、08 AI成本估算、09 文本生成；',
    '- 删除前策分析中“章节卡 + 演示开场等待”的重复段，九环总览后直接进入正式操作演示；',
    '- 前策分析操作后保留/加回 v07 的 PPT 成果页段落；',
    '- 操作演示全部重做包装：界面放大到 1620×912，接近边框尺寸；顶部编号、标题和说明文字整体右移，避免与左侧英文标题重叠；',

...
const outDir = path.join(cwd, '06_预览输出', 'highlights_black_reveal_v01_parts');
const frameDir = path.join(outDir, 'frames');
const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_升级亮点_黑场显形动效_v01_预览.mp4');
const poster = path.join(cwd, '06_预览输出', 'Concetto_2.0_升级亮点_黑场显形动效_v01_检查帧.png');
...
function renderSvg(t, meta, uri) {
  ...
  const titleA = smooth(0.28, 1.05, t) * globalOut;
  const titleWipe = easeOut((t - 0.22) / 1.35);
  const titleSweepX = geometry.x - 310 + (geometry.w + 620) * clamp((t - 0.20) / 1.80);
  const titlePulse = 0.5 + 0.5 * Math.sin(t * 2.1);
  const sparkleIn = smooth(1.18, 1.55, t) * globalOut;

  const sourceCards = [
    { x: 128, y: 258, w: 696, h: 294, iconX: 214, iconY: 338, start: 0.88 },
    { x: 842, y: 258, w: 694, h: 294, iconX: 934, iconY: 338, start: 1.02 },
    { x: 128, y: 566, w: 696, h: 308, iconX: 214, iconY: 648, start: 1.02 },
    { x: 842, y: 566, w: 694, h: 308, iconX: 934, iconY: 648, start: 0.88 },
  ];
  const cards = sourceCards.map((rect) => mapRect(rect, geometry, meta));

  const cardDefs = cards.map((r, i) => {
    const diagonal = Math.hypot(r.w, r.h);
    const reveal = easeOut((t - sourceCards[i].start) / 1.50);
    const radius = 18 + diagonal * 1.08 * reveal;
  ...
  const cardLayers = cards.map((r, i) => {
    ...
    const reveal = easeOut((t - start) / 1.50);
    const alpha = smooth(start, start + 0.24, t) * globalOut;
    const edgeDraw = easeOut((t - (start + 0.52)) / 1.45);
    const breath = 0.5 + 0.5 * Math.sin(t * 1.85 + i * 1.23);
    const finePulse = 0.5 + 0.5 * Math.sin(t * 4.1 + i * 1.52);
    const d = roundedRectPath(r, 28);
    const pathDraw = Math.round(1000 * edgeDraw);
    const orbit = ((t * 0.075 + i * 0.23) % 1) * 1000;
    const glassPhase = ((t * 0.083 + i * 0.17) % 1);
    ...
    return `<g opacity="${alpha.toFixed(3)}">
      ...
      <g clip-path="url(#clip${i})" opacity="${(0.34 * reveal).toFixed(3)}">
        ...
        <rect x="${glassX.toFixed(1)}" y="${(r.y - r.h * 0.24).toFixed(1)}" width="${(r.w * 0.17).toFixed(1)}" height="${(r.h * 1.48).toFixed(1)}" transform="skewX(-18)" fill="url(#glass)" opacity="${(0.13 + 0.08 * breath).toFixed(3)}"/>
        ${sparks}
      </g>
  ...
  <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <defs>
      ...
      <mask id="titleMask" maskUnits="userSpaceOnUse" x="${geometry.x.toFixed(1)}" y="${geometry.y.toFixed(1)}" width="${geometry.w.toFixed(1)}" height="${(geometry.h * 0.27).toFixed(1)}">
        <rect x="${geometry.x.toFixed(1)}" y="${geometry.y.toFixed(1)}" width="${(geometry.w * titleWipe).toFixed(1)}" height="${(geometry.h * 0.27).toFixed(1)}" fill="#ffffff"/>
      </mask>
      ${cardDefs}
    </defs>
    ...
    <rect x="${titleSweepX.toFixed(1)}" y="${(geometry.y + 52).toFixed(1)}" width="350" height="190" transform="skewX(-16)" fill="url(#titleSweep)" opacity="${(0.22 * smooth(0.28, 0.85, t) * (1 - smooth(1.62, 2.0, t))).toFixed(3)}" filter="url(#glow)"/>
...
async function main() {
  ...
  run(ffmpeg, ['-y', '-ss', '3.2', '-i', output, '-frames:v', '1', poster], 'render poster');

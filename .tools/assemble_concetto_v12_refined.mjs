...
const outDir = path.join(cwd, '06_预览输出', 'refined_v12_parts');
...
const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v12_低清.mp4');
const preview = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v12_全片预览.jpg');
const opPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v12_演示包装预览.jpg');
const aiPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_综合调整_v12_AI渲染成果预览.jpg');
const logPath = path.join(cwd, '03_脚本与结构', 'Concetto 2.0_综合调整_v12_说明.md');
...
function operationBgSvg(section) {
  ...
  <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="138" y="112" width="1644" height="928" rx="34" fill="#12102a" opacity="0.72" filter="url(#panelGlow)"/>
    <rect x="138" y="112" width="1644" height="928" rx="34" fill="none" stroke="#8375ff" stroke-opacity="0.42" stroke-width="1.4"/>
    <rect x="160" y="135" width="1600" height="900" rx="24" fill="#f5f5ff" opacity="0.025"/>
    ...
    <text x="508" y="84" font-family="MiSans, PingFang SC, Arial" font-size="24" fill="#8f84dc" opacity="0.82">${section.no}</text>
    <text x="552" y="85" font-family="MiSans, PingFang SC, Arial" font-size="34" font-weight="700" fill="#f3f0ff">${esc(section.title)}</text>
    <text x="740" y="84" font-family="MiSans, PingFang SC, Arial" font-size="20" fill="#aaa2df" opacity="0.92">${esc(section.feature)}</text>
  </svg>`;
}
...
async function renderOperationClip(section, clip = section.clip, feature = section.feature, suffix = '') {
  ...
  const filter = [
    `[0:v]setpts=PTS-STARTPTS,scale=1600:900:force_original_aspect_ratio=decrease,pad=1600:900:(ow-iw)/2:(oh-ih)/2:color=0x17142f,setsar=1,fps=${fps}[vid]`,
    `[1:v]scale=${W}:${H},setsar=1[bg]`,
    `[bg][vid]overlay=x=160:y=135:format=auto[v0]`,
    `[v0]drawbox=x=160:y=135:w=1600:h=900:color=0x8d7cff@0.18:t=2,fade=t=in:st=0:d=0.18,fade=t=out:st=${Math.max(0.1, dur - 0.25)}:d=0.25[v]`,
  ].join(';');
  run([
    '-y',
    '-i', clip,
...
async function main() {
  for (const p of [ffmpeg, ffprobe, fontRegular, fontMedium, fontBold, v10Intro, predesignPpt, music, endCard]) assertFile(p);
  ...
  const records = [{ segment: '开头/升级亮点/新增功能/九环节总览（沿用 v10）', file: v10Intro, duration: probeDur(v10Intro) }];
  ...
  for (const section of sections) {
    ...
    if (section.no === '05') {
      ...
      const op = await renderOperationClip(section);
      parts.push(op);
  ...
  const videoOnly = path.join(outDir, 'video_concat_v12.mp4');
  run(['-y', '-f', 'concat', '-safe', '0', '-i', list, '-c', 'copy', videoOnly], 'concat v12');
  ...
  run(['-y', '-i', output, '-vf', 'fps=0.24,scale=360:-1,tile=10x7', '-frames:v', '1', preview], 'preview v12');
  run(['-y', '-ss', '00:00:36', '-i', output, '-t', '00:00:28', '-vf', 'fps=1,scale=480:-1,tile=7x4', '-frames:v', '1', opPreview], 'operation preview v12');
  run(['-y', '-ss', '00:01:34', '-i', output, '-t', '00:00:38', '-vf', 'fps=1,scale=480:-1,tile=10x4', '-frames:v', '1', aiPreview], 'ai preview v12');

  fs.writeFileSync(logPath, [
    '# Concetto 2.0 综合调整 v12',
    '',
    '本版按最新反馈处理：',
    '',
    '- 开头、升级亮点、新增功能、九环节总览沿用 v10，不重绘现成素材字体；',
    '- 九个环节开头恢复为 v05/图三方向：暗场、漂浮界面、中心真实功能卡；顺序保持 01 前策分析、02 场地定位、03 图生模型、04 数智建模、05 AI灵感渲染、06 总图排布、07 AI仿真分析、08 AI成本估算、09 文本生成；',
    '- 前策分析操作后加回 v07 的 PPT 成果页段落；',
    '- 操作演示全部重做包装：界面放大到 1690×820，取消旧版黑框感，说明配字放到底部信息区；',
    '- AI 灵感渲染成果部分重做为暗场发布会风格展示，保留原始素材和文字内容。',
    '',
    `输出视频：\`${output}\``,
    `全片预览：\`${preview}\``,
    `演示包装预览：\`${opPreview}\``,

...
const outDir = path.join(cwd, '06_预览输出', 'highlights_black_reveal_v03_parts');
const frameDir = path.join(outDir, 'frames');
const visualOutput = path.join(outDir, 'highlight_global_breath_v03_video_only.mp4');
const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_升级亮点_全画面呼吸_v03_对话内预览.mp4');
...
function renderSvg(t, meta, uri) {
  ...
  <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="${(54 - 2.5 * sceneBreath).toFixed(1)}" y="${(38 - 1.5 * sceneBreath).toFixed(1)}" width="${(1812 + 5 * sceneBreath).toFixed(1)}" height="${(1004 + 3 * sceneBreath).toFixed(1)}" rx="34" fill="none" stroke="#5f4fe0" stroke-width="${(0.9 + 0.35 * sceneBreath).toFixed(2)}" stroke-opacity="${((0.075 + 0.055 * sceneBreath) * globalA).toFixed(3)}" filter="url(#pinGlow)"/>
    <rect x="${(126 + 12 * (1 - sceneBreath2)).toFixed(1)}" y="1018" width="${(W - 252 - 24 * (1 - sceneBreath2)).toFixed(1)}" height="1.0" fill="url(#breathLine)" opacity="${(0.055 * breathGate * (0.48 + 0.52 * sceneBreath2)).toFixed(3)}"/>
  </svg>`;
}

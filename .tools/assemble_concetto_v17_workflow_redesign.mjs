...
const outDir = path.join(cwd, '06_预览输出', 'refined_v16_parts');
...
const openingClip = path.join(cwd, '开头', 'Concetto_2.0_开头登场视觉_v01_9s.mp4');
const highlightDir = path.join(cwd, '亮点');
const highlightImages = fs.existsSync(highlightDir)
  ? fs.readdirSync(highlightDir)
...
const output = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环重制_v17_低清.mp4');
const preview = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环重制_v17_全片预览.jpg');
const opPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环重制_v17_演示包装预览.jpg');
const aiPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环重制_v17_AI渲染成果预览.jpg');
const introPreview = path.join(cwd, '06_预览输出', 'Concetto_2.0_九环重制_v17_前置段检查.jpg');
const logPath = path.join(cwd, '03_脚本与结构', 'Concetto 2.0_九环重制_v17_说明.md');
...
function workflowGeom(t, dur) {
  ...
  const imgW = 1810 * (0.975 + 0.025 * panel);
  const imgH = imgW * 9 / 16;
  const imgX = (W - imgW) / 2;
  const imgY = 36 - 6 * panel;
  const sx = imgW / 2048;
  const sy = imgH / 1152;
...
function renderWorkflowOverviewSvg(t, dur = 7.2) {
  ...
  const scan = -620 + (W + 1180) * (t / dur);
  const progress = smooth(0.95, dur - 0.85, t);
  ...
  const glows = nodes.map((n, i) => {
    const on = i < litCount;
    const near = Math.max(0, 1 - Math.hypot(light.x - n.x, light.y - n.y) / 210);
    const pulse = on ? (0.46 + 0.18 * Math.sin(t * 5.4 + i)) : 0;
    return `<g opacity="${(a * (on ? 1 : 0)).toFixed(3)}">
      <circle cx="${n.x.toFixed(1)}" cy="${n.y.toFixed(1)}" r="${(63 + pulse * 14 + near * 24).toFixed(1)}" fill="#9271ff" opacity="${(0.12 + pulse * 0.12 + near * 0.20).toFixed(3)}" filter="url(#glow)"/>
      <circle cx="${n.x.toFixed(1)}" cy="${n.y.toFixed(1)}" r="${(45 + near * 10).toFixed(1)}" fill="none" stroke="#ffffff" stroke-width="${(1.1 + near * 1.4).toFixed(2)}" opacity="${(0.10 + near * 0.36).toFixed(3)}" filter="url(#glow)"/>
    </g>`;
  }).join('');
  const entrance = easeOut(t / 1.3);
  const stageScale = 0.992 + 0.008 * entrance;
  const cx = W / 2;
  const cy = H / 2 + 6;
  return `<?xml version="1.0" encoding="UTF-8"?>
  <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ${defs()}
    <rect width="${W}" height="${H}" fill="#010104"/>
    <rect width="${W}" height="${H}" fill="url(#core)" opacity="${(0.62 * a).toFixed(3)}"/>
    <path d="M 112 836 C 520 748, 1180 780, 1808 690" stroke="#7668ff" stroke-opacity="${(0.16 * a).toFixed(3)}" stroke-width="1.1" fill="none"/>
    ...
    <rect x="${scan.toFixed(1)}" y="-80" width="560" height="${H + 160}" transform="skewX(-18)" fill="url(#lineGrad)" opacity="${(0.072 * a).toFixed(3)}"/>
    ...
    <g opacity="${a.toFixed(3)}" transform="translate(${cx} ${cy}) scale(${stageScale.toFixed(4)}) translate(${-cx} ${-cy})">
      ...
      <image href="${imgUri}" x="${imgX.toFixed(1)}" y="${imgY.toFixed(1)}" width="${imgW.toFixed(1)}" height="${imgH.toFixed(1)}" preserveAspectRatio="xMidYMid meet" opacity="0.985"/>
    </g>
    <g opacity="${a.toFixed(3)}" transform="translate(${cx} ${cy}) scale(${stageScale.toFixed(4)}) translate(${-cx} ${-cy})">
      <polyline points="${fullPts}" fill="none" stroke="#7668ff" stroke-opacity="0.10" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
      <polyline points="${activePts}" fill="none" stroke="#ffffff" stroke-opacity="0.64" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" filter="url(#glow)"/>
      <polyline points="${activePts}" fill="none" stroke="#8d71ff" stroke-opacity="0.45" stroke-width="9" stroke-linecap="round" stroke-linejoin="round" filter="url(#glow)"/>
      ${glows}
      <circle cx="${light.x.toFixed(1)}" cy="${light.y.toFixed(1)}" r="9" fill="#ffffff" opacity="${(0.88 * a).toFixed(3)}" filter="url(#glow)"/>
      <circle cx="${light.x.toFixed(1)}" cy="${light.y.toFixed(1)}" r="30" fill="#b58cff" opacity="${(0.20 * a).toFixed(3)}" filter="url(#glow)"/>
    </g>
  </svg>`;
}
...
async function renderWorkflowOverview() {
  const dur = 7.2;
  const dir = path.join(frameRoot, 'workflow_overview_v17');
  fs.mkdirSync(dir, { recursive: true });
  const count = Math.round(dur * fps);
  for (let i = 0; i < count; i++) {
    const out = path.join(dir, `frame_${String(i).padStart(4, '0')}.png`);
  ...
  const out = path.join(outDir, 'intro_workflow_overview_v17.mp4');
  run(['-y', '-framerate', String(fps), '-i', path.join(dir, 'frame_%04d.png'), '-c:v', 'libx264', '-preset', 'medium', '-crf', '20', '-pix_fmt', 'yuv420p', '-r', String(fps), out], 'workflow overview v17');
  return out;
}

function renderOpeningFromFolder() {
  const out = path.join(outDir, 'intro_opening_folder_direct_v16.mp4');
  const dur = probeDur(openingClip);
  run([
    '-y',
    '-i', openingClip,
    '-vf', `fps=${fps},scale=${W}:${H}:force_original_aspect_ratio=decrease,pad=${W}:${H}:(ow-iw)/2:(oh-ih)/2:color=black,setsar=1,fade=t=out:st=${Math.max(0.1, dur - 0.32).toFixed(2)}:d=0.32`,
    '-an',
    '-c:v', 'libx264',
    '-preset', 'medium',
    '-crf', '20',
...
function highlightCardRects(imgX, imgY, imgW, imgH, meta) {
  ...
  const rects = [
    { x: 515, y: 913, w: 2188, h: 910, iconX: 780, iconY: 1170 },
    { x: 2765, y: 913, w: 2188, h: 910, iconX: 3030, iconY: 1170 },
    { x: 515, y: 1888, w: 2188, h: 960, iconX: 780, iconY: 2160 },
    { x: 2765, y: 1888, w: 2188, h: 960, iconX: 3030, iconY: 2160 },
  ];
  return rects.map((r) => ({
    x: imgX + r.x * sx,
    y: imgY + r.y * sy,
...
function renderHighlightSvg(t, dur, imgFile, meta, index = 0) {
  const imgUri = dataUri(imgFile);
  const a = smooth(0.08, 0.85, t) * (1 - smooth(dur - 0.55, dur, t));
  const panel = easeOut(t / 1.15);
  ...
  const scan = -560 + (W + 1050) * (t / dur);
  const cardGlows = cards.map((r, i) => {
    const k = smooth(0.75 + i * 0.55, 1.35 + i * 0.55, t) * (1 - smooth(dur - 0.8, dur - 0.25, t));
    const pulse = 0.55 + 0.45 * Math.sin(t * 4.2 + i * 1.7);
    const sweepX = r.x - r.w * 0.35 + (r.w * 1.7) * clamp((t - (1.25 + i * 0.42)) / 1.65);
    return `<g opacity="${(a * k).toFixed(3)}">
      <rect x="${r.x.toFixed(1)}" y="${r.y.toFixed(1)}" width="${r.w.toFixed(1)}" height="${r.h.toFixed(1)}" rx="26" fill="none" stroke="#ffffff" stroke-width="2.2" stroke-opacity="${(0.28 + pulse * 0.25).toFixed(3)}" filter="url(#glow)"/>
      <rect x="${sweepX.toFixed(1)}" y="${(r.y - 30).toFixed(1)}" width="${(r.w * 0.22).toFixed(1)}" height="${(r.h + 60).toFixed(1)}" transform="skewX(-18)" fill="url(#lineGrad)" opacity="${(0.16 * k).toFixed(3)}"/>
      <circle cx="${r.iconX.toFixed(1)}" cy="${r.iconY.toFixed(1)}" r="${(54 + pulse * 10).toFixed(1)}" fill="#9b7dff" opacity="${(0.18 + pulse * 0.16).toFixed(3)}" filter="url(#glow)"/>
    </g>`;
  }).join('');
  const orbit = cards.map((r, i) => {
    const k = smooth(1.25 + i * 0.4, 2.05 + i * 0.4, t);
    const cx = r.x + r.w * (0.68 + 0.08 * Math.sin(t * 1.5 + i));
    const cy = r.y + r.h * (0.50 + 0.16 * Math.cos(t * 1.25 + i));
    return `<circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="${(4 + k * 3).toFixed(1)}" fill="#ffffff" opacity="${(a * k * 0.7).toFixed(3)}" filter="url(#glow)"/>`;
  }).join('');
  return `<?xml version="1.0" encoding="UTF-8"?>
  <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ${defs()}
    <rect width="${W}" height="${H}" fill="#010104"/>
    <rect width="${W}" height="${H}" fill="url(#core)" opacity="${(0.72 * a).toFixed(3)}"/>
    <rect x="${scan.toFixed(1)}" y="0" width="640" height="${H}" transform="skewX(-18)" fill="url(#lineGrad)" opacity="${(0.095 * a).toFixed(3)}"/>
    <path d="M 105 830 C 570 748, 1200 778, 1810 684" stroke="#7668ff" stroke-opacity="${(0.20 * a).toFixed(3)}" stroke-width="1.2" fill="none"/>
    <text x="112" y="118" font-family="Avenir Next, Arial" font-size="16" letter-spacing="7" fill="#857be8" opacity="${(0.70 * a).toFixed(3)}">CONCETTO 2.0 / FEATURE HIGHLIGHTS</text>
    <rect x="112" y="140" width="${(280 * a).toFixed(1)}" height="1.2" fill="#7668ff" opacity="0.55"/>
    <g opacity="${a.toFixed(3)}" filter="url(#glow)">
      <rect x="${(imgX - 24).toFixed(1)}" y="${(imgY - 24).toFixed(1)}" width="${(imgW + 48).toFixed(1)}" height="${(imgH + 48).toFixed(1)}" rx="42" fill="#151039" stroke="#8d7cff" stroke-opacity="0.24"/>
    ...
    <g opacity="${a.toFixed(3)}">
      ${cardGlows}
      ${orbit}
      <rect x="${(imgX + imgW * 0.02).toFixed(1)}" y="${(imgY + imgH * 0.05).toFixed(1)}" width="${(imgW * 0.96).toFixed(1)}" height="2" fill="url(#lineGrad)" opacity="${(0.32 * smooth(0.5, 1.7, t)).toFixed(3)}" filter="url(#glow)"/>
    </g>
  </svg>`;
}

async function renderHighlightsIntro() {
  const parts = [];
  for (let idx = 0; idx < highlightImages.length; idx++) {
    const img = highlightImages[idx];
    const meta = await sharp(img).metadata();
    const slim = path.join(outDir, `highlight_source_${String(idx + 1).padStart(2, '0')}_v16.jpg`);
    await sharp(img).resize(1920, 1080, { fit: 'inside', withoutEnlargement: true }).jpeg({ quality: 92 }).toFile(slim);
    const dur = 7.2;
    const dir = path.join(frameRoot, `highlight_${String(idx + 1).padStart(2, '0')}_v16`);
    fs.mkdirSync(dir, { recursive: true });
    const count = Math.round(dur * fps);
    for (let i = 0; i < count; i++) {
      const out = path.join(dir, `frame_${String(i).padStart(4, '0')}.png`);
      const t = i / fps;
      await sharp(Buffer.from(renderHighlightSvg(t, dur, slim, meta, idx))).png().toFile(out);
    }
    const out = path.join(outDir, `intro_highlight_${String(idx + 1).padStart(2, '0')}_v16.mp4`);
    run(['-y', '-framerate', String(fps), '-i', path.join(dir, 'frame_%04d.png'), '-c:v', 'libx264', '-preset', 'medium', '-crf', '20', '-pix_fmt', 'yuv420p', '-r', String(fps), out], `highlight intro ${idx + 1} v16`);
    parts.push(out);
  }
  return parts;
}
...
async function main() {
  for (const p of [ffmpeg, ffprobe, fontRegular, fontMedium, fontBold, openingClip, predesignPpt, music, endCard, workflowImage, meetingClip]) assertFile(p);
  if (highlightImages.length === 0) throw new Error(`亮点文件夹没有可用图片：${highlightDir}`);
  for (const p of highlightImages) assertFile(p);
  ...
  const previousList = path.join(outDir, 'concat_list.txt');
  assertFile(previousList);
  const parts = fs.readFileSync(previousList, 'utf8')
    .split(/\n+/)
    .map((line) => line.trim())
    .filter(Boolean)
    .map((line) => line.replace(/^file\s+'/, '').replace(/'$/, '').replace(/'\\''/g, "'"))
    .map((p) => /intro_workflow_overview_v\d+\.mp4$/.test(p) ? workflowOverview : p);
  const records = [
    { segment: '前置段 01：开头文件夹视频直接使用（沿用 v16）', file: parts[0], duration: probeDur(parts[0]) },
    { segment: '前置段 02：亮点图片动效（沿用 v16）', file: parts[1], duration: probeDur(parts[1]) },
    { segment: '前置段 03：九大环节最新版图片重新组织动效 v17', file: workflowOverview, duration: probeDur(workflowOverview) },
  ];

  for (let i = 3; i < parts.length; i++) {
    records.push({ segment: `主体段落沿用 v16 #${String(i - 2).padStart(2, '0')}`, file: parts[i], duration: probeDur(parts[i]) });
  }

  const list = path.join(outDir, 'concat_list.txt');
  fs.writeFileSync(list, parts.map((p) => `file '${quoteFile(p)}'`).join('\n') + '\n');
  const videoOnly = path.join(outDir, 'video_concat_v17.mp4');
  run(['-y', '-f', 'concat', '-safe', '0', '-i', list, '-c', 'copy', videoOnly], 'concat v17');
  ...
  run(['-y', '-i', output, '-vf', 'fps=0.24,scale=360:-1,tile=10x7', '-frames:v', '1', preview], 'preview v17');
  run(['-y', '-ss', '00:00:00', '-i', output, '-t', '00:00:32', '-vf', 'fps=1,scale=480:-1,tile=8x4', '-frames:v', '1', introPreview], 'intro preview v17');
  run(['-y', '-ss', '00:00:31', '-i', output, '-t', '00:00:32', '-vf', 'fps=1,scale=480:-1,tile=8x4', '-frames:v', '1', opPreview], 'operation preview v17');
  run(['-y', '-ss', '00:01:24', '-i', output, '-t', '00:00:42', '-vf', 'fps=1,scale=480:-1,tile=10x5', '-frames:v', '1', aiPreview], 'ai preview v17');

  fs.writeFileSync(logPath, [
    '# Concetto 2.0 九大环节重制 v17',
    '',
    '本版按最新反馈处理：',
    '',
    '- 前策分析之前的内容完整重做，不再使用旧 intro 中的亮点/总览选段；',
    '- 开头段直接使用 `开头` 文件夹中的视频素材；',
    '- 亮点段使用 `亮点` 文件夹中的图片素材重新生成，加入卡片边缘扫光、图标呼吸光晕与全局扫描光；',
    '- 九大环节总览使用 `九大环节最新版` 图片重新组织画面，不再沿用旧屏幕框效果；',
    '- 新九环段加入更克制的路径能量线、节点呼吸光晕与细扫光，视觉语法贴近开头与亮点段；',
    '- 01 前策分析章节开场重新生成，保持与后续 02–09 章节开头一致；',
    '- 其余主体段落沿用 v15 的现有规划与素材配置。',
    '',
    `输出视频：\`${output}\``,
    `全片预览：\`${preview}\``,
    `前置段检查：\`${introPreview}\``,

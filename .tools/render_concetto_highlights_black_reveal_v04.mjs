...
const outDir = path.join(cwd, '06_预览输出', 'highlights_black_reveal_v04_parts');
...
const visualOutput = path.join(outDir, previewMode ? 'highlight_luminous_edges_v04_preview_video_only.mp4' : 'highlight_luminous_edges_v04_2560p60_video_only.mp4');
const output = path.join(cwd, '06_预览输出', previewMode ? 'Concetto_2.0_升级亮点_精细发光边缘_v04_低清预览.mp4' : 'Concetto_2.0_升级亮点_精细发光边缘_v04_2560p60.mp4');
const DUR = 9.4;
...
function renderSvg(t, meta, uri) {
  ...
  const cardLayers = cards.map((r, i) => {
    ...
    return `<g>
      ...
      <g clip-path="url(#textClip${i})" opacity="${((0.11 + 0.12 * textBreath) * reveal).toFixed(3)}">
      ...
      <path d="${d}" pathLength="1000" fill="none" stroke="#5e48ff" stroke-width="${((11.5 + 3.5 * edgeBreath) * sx).toFixed(2)}" stroke-opacity="${((0.085 + 0.075 * edgeBreath) * edgeDraw).toFixed(3)}" filter="url(#glow)"/>
      <path d="${d}" pathLength="1000" fill="none" stroke="#7967ff" stroke-width="${((5.0 + 1.4 * edgeBreath) * sx).toFixed(2)}" stroke-linecap="round" stroke-linejoin="round" stroke-dasharray="${pathDraw} 1000" stroke-opacity="${(0.25 + 0.28 * edgeBreath * edgeDraw).toFixed(3)}" filter="url(#pinGlow)"/>
      <path d="${d}" pathLength="1000" fill="none" stroke="url(#edge)" stroke-width="${((3.15 + 0.85 * edgeBreath) * sx).toFixed(2)}" stroke-linecap="round" stroke-linejoin="round" stroke-dasharray="${pathDraw} 1000" stroke-opacity="${(0.54 + 0.38 * edgeDraw).toFixed(3)}" filter="url(#pinGlow)"/>
      <path d="${d}" pathLength="1000" fill="none" stroke="#ffffff" stroke-width="${((1.05 + 0.55 * finePulse) * sx).toFixed(2)}" stroke-linecap="round" stroke-linejoin="round" stroke-opacity="${((0.20 + 0.22 * edgeBreath) * edgeDraw).toFixed(3)}"/>
  ...
  <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <g clip-path="url(#titleClip)" mask="url(#titleMask)" opacity="${((0.18 + 0.18 * sceneBreath) * titleA).toFixed(3)}">
